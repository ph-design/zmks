/* SPIRAL IN effect (center moves inward) */
static float spiral_in_center = 0.0f;
static void zmk_rgb_underglow_effect_spiral_in(void) {
    ...
    spiral_in_center -= (float)state.animation_speed * 0.4f;
    if (spiral_in_center < 0.0f)
        spiral_in_center += STRIP_NUM_PIXELS;
}

static void spiral_in_reset(void) {
    spiral_in_center = 0.0f;
}
